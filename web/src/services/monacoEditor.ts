...
export function useMonacoEditor(
...
): { content: Ref<string>; errorMarkers: Ref<monaco.editor.IMarker[]> } {
    const content = ref('');
    ...
    if (!scope) {
        throw new Error('useMonacoEditor called without a scope');
    }
        ...
        (window as any).require(['vs/editor/editor.main'], function () {
            ...
            if (config?.javascriptLib) {
                for (const k of Object.getOwnPropertyNames(config.javascriptLib)) {
                    ...
                    if (
                        thisMonaco.languages.typescript.javascriptDefaults.getExtraLibs()[
                            libUrl
                        ]
                    ) {
                        continue;
                    ...
                    thisMonaco.languages.typescript.javascriptDefaults.addExtraLib(
                        lib,
                        libUrl
                    );
                    thisMonaco.editor.createModel(
                        lib,
                        'typescript',
                        thisMonaco.Uri.parse(libUrl)
                    );
                }
            }
